...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        self.lora_A[adapter_name] = Linear(self.in_features, r, bias=False)
        self.lora_B[adapter_name] = Linear(r, self.out_features, bias=False)
        if use_rslora:
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        ...
        if adapter_name in self.lora_A.keys():
            if init_lora_weights is True:
                # initialize A the same way as the default for Linear and B to zero
...
def dispatch_default(
    ...
    **kwargs,
    ...
    elif isinstance(target_base_layer, Linear):
        if kwargs["fan_in_fan_out"]:
            warnings.warn(
                "fan_in_fan_out is set to True but the target module is `Linear`. "
