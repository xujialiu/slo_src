...
from util.datasets_energy import (
    build_dataset_in,
    build_dataset_out,
...
from engine_finetune_energy_mc_dropout_entropy_2 import train, evaluate, evaluate_out
...
from mc_dropout_entropy_2 import MCDropout
...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        default=None,
        choices=["fp", "slo"],
        help="dataset type (fp or slo, default: None)",
    )
    ...
    # energy ood
    parser.add_argument("--T", default=1.0, type=float, help="temperature: energy ood")
    parser.add_argument("--csv_path_out", help="path where csv file is located")
    parser.add_argument("--data_path_out", help="path where images are located")
...
def main(args):
    ...
    if args.train_all:
        dataset_train_in = build_dataset_in(is_train="all", args=args)
    ...
    if args.eval:
        dataset_test = build_dataset_in(is_train=args.test_name, args=args)
    ...
    if args.use_weighted_sampler:
        try:
            sampler_train_in = get_weighted_sampler(dataset_train_in)
            print(f"use weighted sampler")
        except BaseException:
            sampler_train_in = torch.utils.data.DistributedSampler(
                dataset_train_in,
        ...
        print(f"use distributed sampler")
    print(f"Sampler_train = {sampler_train_in}")

    try:
        sampler_train_out = torch.utils.data.RandomSampler(
            dataset_train_out, replacement=True, num_samples=len(dataset_train_in)
        )
    except ValueError:
        sampler_train_out = None
    sampler_val_out = torch.utils.data.SequentialSampler(dataset_val_out)

    # create val sampler
    if args.dist_eval:
        if len(dataset_val_in) % num_tasks != 0:
            print(
        ...
        sampler_val = torch.utils.data.DistributedSampler(
            dataset_val_in, num_replicas=num_tasks, rank=global_rank, shuffle=False
        )
    else:
        sampler_val = torch.utils.data.SequentialSampler(dataset_val_in)
    ...
    data_loader_train_in = torch.utils.data.DataLoader(
        dataset_train_in,
        sampler=sampler_train_in,
        batch_size=args.batch_size,
        num_workers=args.num_workers,
        pin_memory=args.pin_mem,
        drop_last=True,
    )
    data_loader_train_out = torch.utils.data.DataLoader(
        dataset_train_out,
        sampler=sampler_train_out,
        batch_size=args.batch_size,
        num_workers=args.num_workers,
        pin_memory=args.pin_mem,
        drop_last=True,
    ...
    data_loader_val_in = torch.utils.data.DataLoader(
        dataset_val_in,
        sampler=sampler_val,
        batch_size=args.batch_size,
        num_workers=args.num_workers,
    ...
    data_loader_val_out = torch.utils.data.DataLoader(
        dataset_val_out,
        sampler=sampler_val_out,
        batch_size=args.batch_size,
        num_workers=args.num_workers,
        pin_memory=args.pin_mem,
        drop_last=False,
    ...
    if "large" in args.model:
        kwargs = EasyDict(embed_dim=1024, depth=24, num_heads=16, fusion_num_heads=16)
    elif "base" in args.model:
        kwargs = EasyDict(embed_dim=768, depth=12, num_heads=12, fusion_num_heads=12)
    ...
    if args.lora_position == "all":
        ...
        target_modules = r"image_encoder\.blocks\.\d+\.attn\.qkv$"
    ...
    for name, param in model.named_parameters():
        ...
        elif name.startswith("fusion_block") or ("head" in name):
            param.requires_grad = True
    ...
    class_percentages = (
        dataset_train_in.class_counts / dataset_train_in.class_counts.sum()
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        if args.distributed:
            data_loader_train_in.sampler.set_epoch(epoch)
        train_stats = train(
            model,
            criterion,
            data_loader_train_in,
            data_loader_train_out,
            optimizer,
            device,
            epoch,
            loss_scaler,
        ...
        (
            ...
            args=args,
            data_loader=data_loader_val_in,
            model=model,
            device=device,
            epoch=epoch,
            mode="val",
        ...
        evaluate_out(
            args=args,
            data_loader=data_loader_val_out,
            model=model,
            device=device,
            epoch=epoch,
        )
        ...
        if args.save_best_ckpt and (max_auc < val_auc_roc):
            max_auc = val_auc_roc
            if args.output_dir:
                misc.save_model(
                    args=args,
