    ...
    def forward(
        ...
        if self.on_batch:
            ...
            for i in range(b):
                ...
                logists = self.core_model(x_)

                y_pred = logists.argmax(dim=-1)

                y_pred_mode, counts = torch.unique(y_pred, return_counts=True)

                y_idx = torch.where(y_pred == y_pred_mode[0])[0]

                y_final = logists[y_idx, :].mean(dim=0)

                # uncertainty = counts[0] / self.num_estimators

                list_logists_final.append(y_final)
                
                list_logits.append(logists)
                # list_uncertainty.append(uncertainty)

            logists_final = torch.stack(list_logists_final, dim=0)
            probs = torch.softmax(torch.stack(list_logits, dim=0), axis=-1)
            per_sample_entropy = -torch.sum(probs * torch.log(probs + 1e-9), dim=-1)
            uncertainty = per_sample_entropy.mean(axis=-1)
            # uncertainty = torch.stack(list_uncertainty, dim=0)
            # y.flatten()
            # probs = torch.softmax(predictions, dim=-1)  # [n_estimates, C]
            # mean_probs = probs.mean(dim=0)               # [C]
            # entropy = -torch.sum(mean_probs * torch.log(mean_probs + 1e-9))  # 熵越高越不确定
            

            return logists_final, uncertainty
...
def uncertainty_metrics(probs):
    # 平均熵
    avg_entropy = -torch.sum(probs * torch.log(probs + 1e-9), dim=-1).mean()
    
    # 类间分歧
    pred_classes = probs.argmax(dim=-1)
    consensus_rate = torch.mode(pred_classes)[1].float() / probs.shape[0]
    disagreement = 1 - consensus_rate
    
    # 互信息
    mean_probs = probs.mean(dim=0)
    predictive_entropy = -torch.sum(mean_probs * torch.log(mean_probs + 1e-9))
    mutual_info = predictive_entropy - (-torch.sum(probs * torch.log(probs + 1e-9), dim=-1).mean())
