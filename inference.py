...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument(
        "--data_path", default="./OCT_data/", type=str, help="dataset path"
    )
    parser.add_argument(
        "--nb_classes", default=16, type=int, help="number of the classification types"
    )
    ...
    parser.add_argument(
        "--save_path",
        default="./trained/FMUE.pth",
        help="path where to save model path",
    )
    parser.add_argument(
        "--method", default="FMUE", help="path where to save the test result"
    )
...
def val(val_dataloader, model, args, mode, device):
    ...
    with torch.no_grad():
        for i, img_data_list in enumerate(tbar):
            ...
            one_hot = (
                ...
                .to(device)
                .scatter_(1, cls_label.unsqueeze(1), 1)
            )
            ...
            for idx in range(data_bach):
                ...
                ground_truths.append(cls_label.cpu().detach().float().numpy()[idx])
    ...
    F1Score = f1_score(ground_truths, predictions, average="macro")

    print(
        f"{mode} == Acc: {round(Acc, 6)}, AUC: {round(epoch_auc, 6)}, F1Score: {round(F1Score, 6)}\n"
    )
...
def main(args=None):
    ...
    label_list_return, predict_return, preds_confu, label_confu = val(
        test_loader, model, args, mode="test", device=device
    )
    ...
    for i in range(args.nb_classes):
        fpr_Pri[i], tpr_Pri[i], _ = roc_curve(
            label_list_return[:, i], predict_return[:, i]
        )
        roc_auc_Pri[i] = auc(fpr_Pri[i], tpr_Pri[i])

    # Compute micro-average ROC curve and ROC area
    fpr_Pri["micro"], tpr_Pri["micro"], _ = roc_curve(
        label_list_return.ravel(), predict_return.ravel()
    )
    ...
    plt.plot(
        fpr_Pri["macro"],
        tpr_Pri["macro"],
        label="Average (AUC = {0:0.4f})" "".format(roc_auc_Pri["macro"]),
        color="navy",
        linestyle=":",
        linewidth=4,
    )
