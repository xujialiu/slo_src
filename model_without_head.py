...
class MultiScale(nn.Module):
    def __init__(
        ...
        if num_head_layers == 3:
            ...
            self.head_icdr_01 = nn.Sequential(
                nn.Linear(channels[0], channels[1]),
                nn.GELU(),
                nn.Dropout(p=0.1),
                nn.Linear(channels[1], channels[2]),
            ...
            self.head_icdr_4 = nn.Sequential(
                nn.Linear(channels[0], channels[1]),
                nn.GELU(),
                nn.Dropout(p=0.1),
                nn.Linear(channels[1], channels[2]),
    ...
    def forward(self, x):
        ...
        x_dme = self.head_dme(x)
        x_icdr_01 = self.head_icdr_01(x)
        x_icdr_4 = self.head_icdr_4(x)
        return x_dme, x_icdr_01, x_icdr_4
