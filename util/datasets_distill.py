...
def build_dataset(is_train, args):
    ...
    transform, transform_model, transform_teacher_model = build_transform(
        is_train, args
    )
    dataset = LoadData(
        ...
        transform_teacher_model=transform_teacher_model,
    )
    return dataset
...
class LoadData(Dataset):
    def __init__(
        ...
        self.transform_teacher_model = transform_teacher_model
    ...
    def __getitem__(self, index):
        ...
        img_model = self.transform_model(image=img)["image"]
        img_teacher = self.transform_teacher_model(image=img)["image"]

        return (img_model, img_teacher)
...
def build_transform_slo(is_train, args):
    ...
    # resize image size for uniformize image size
    image_height, image_width = args.resize_to
    ...
    transform_teacher_model = A.Compose(
        [
            A.Resize(
                height=args.fm_input_size,
                width=args.fm_input_size,
                interpolation=cv2.INTER_CUBIC,
            ),
            A.Normalize(mean=mean, std=std),
            ToTensorV2(),
    ...
    return transform, transform_model, transform_teacher_model
