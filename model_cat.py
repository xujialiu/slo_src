...
class MultiScale(nn.Module):
    def __init__(
        ...
        self.head = (
            nn.Linear(embed_dim+fusion_num_heads, num_classes) if num_classes > 0 else nn.Identity()
        )
...
class FusionBlock(nn.Module):
    def __init__(
        ...
        #     # self.fusion_pos_embed = torch.zeros(
        #     #     1,
        #     #     total_patches,
        #     #     embed_dim,
        #     # ).to("cuda:0")
        ...
        self.transformer_blocks = nn.Sequential(
            *[
                TransformerBlock(
                    dim=embed_dim + fusion_num_heads, # embed_dim = 768
                    num_heads=fusion_num_heads, # 12
        ...
        self.fusion_norm = nn.LayerNorm(embed_dim + fusion_num_heads, eps=1e-6)
    ...
    def forward(self, x):
        ...
        B = x.shape[0]
        x = torch.cat([x, self.fusion_pos_embed.expand(B, -1, -1)], dim=-1)
