...
def evaluate(args, data_loader, model, device, epoch, mode, num_class):
    ...
    for data_iter_step, batch in enumerate(
        ...
        # compute output
        with torch.cuda.amp.autocast():
            output, uncertainty = model(image)
    ...
    with open(
        ...
        if epoch == 0:
            txt.write(
                f"Mode,Epoch,AUC_macro,AUC_weighted,F1,Kappa,Accuracy,Precision,Sensitivity,Specificity\n"
            )
    ...
    # )
    print(
        f"{mode} Epoch {epoch}: AUC macro: {auc_macro}, AUC weighted: {auc_weighted}, F1: {f1}, Kappa: {kappa}, Accuracy: {accuracy}, Precision: {precision}, Sensitivity: {sensitivity}, Specificity: {specificity}\n"
    )
